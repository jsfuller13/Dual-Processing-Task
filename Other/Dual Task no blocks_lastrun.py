...
expName = 'Dual Task no blocks'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Jordan\\Documents\\Experiment Building\\2. Dual Processing Task\\Dual Task no blocks_lastrun.py',
...
B1_trials = data.TrialHandler(nReps=1, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('DPT_B1.csv'),
...
for thisB1_trial in B1_trials:
    ...
    # update component parameters for each repeat
    sound_1.setSound(SoundFile, secs=2)
    sound_1.setVolume(1, log=False)
    # keep track of which components have finished
    trialComponents = [sound_1, prompt]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if sound_1.status == STARTED and t >= frameRemains:
            ...
            if 2 > 0.5:  # don't force-stop brief sounds
                sound_1.stop()
...
if B1_trials.trialList in ([], [None], None):
    params = []
else:
    params = B1_trials.trialList[0].keys()
# save data for this loop
B1_trials.saveAsExcel(filename + '.xlsx', sheetName='B1_trials',
    stimOut=params,
...
# get names of stimulus parameters
if B3_trials.trialList in ([], [None], None):
    params = []
else:
    params = B3_trials.trialList[0].keys()
# save data for this loop
B3_trials.saveAsExcel(filename + '.xlsx', sheetName='B3_trials',
    stimOut=params,
    dataOut=['n','all_mean','all_std', 'all_raw'])
